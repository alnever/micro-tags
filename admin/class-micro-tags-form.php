...
class Micro_Tags_Form
{
  ...
  private function set_db_options($table_id)
  {
    ...
    $this->db_table_info = Micro_Tags_Db::get_instance()->get_tables()[$table_id];
    $this->db_table = (new Micro_Tags_Db_Table)->get_table($table_id);
 ...
 */
    ...
    if(isset($_REQUEST['nonce']))
    {
      ...
      if ($item_valid === true) {
          if ($this->item[$this->id_field] == 0) {
              ...
              if ($result) {
                  $this->message = __('Запись успешно добавлена', 'micro-tags-form');
              } else {
                  $this->notice = __('Во время добавления записи возникли ошибки!', 'micro-tags-form');
              }
          } else {
              $result = $this->update();

              if ($result) {
                  $this->message = __('Запись успешно изменена', 'micro-tags-form');
              } else {
                  $this->notice = __('Во время изменения записи возникли ошибки!', 'micro-tags-form');
              }
          }
      } else {
      ...
      if (isset($_REQUEST[$this->id_field])) {
          ...
          if (!$this->item) {
              $this->item = $this->set_defaults();
              $this->notice = __('Запись не найдена', 'micro-tags-form');
          }
      }
    }
  }
  ...
  private function validate($item)
  {
      ...
      foreach ($this->db_table->get_fields() as $key => $value) {
        if ($value["display"] && $value["required"])
        {
          if (empty($item[$key]))
          {
            $messages[] = __("Поле необходимо заполнить ".$key." ".$value["caption"], 'micro-tags-form');
          }
        }
      }
  ...
  {
    echo sprintf('<input id="%s" name="%s" type="text" value="%d" class="easyui-numberbox" style="width:400px
                   placeholder="%s" %s />',
                   $field["name"],
                   $field["name"],
                   $this->item[$field["name"]],
  ...
  public function show_input_int_combo($field)
  {
    ...
    echo sprintf('<select id="%s" name="%s" class="easyui-combobox" placeholder="%s" %s style="width:400px;z-index:0">',
                   $field["name"],
                   $field["name"],
                   $field["caption"],
                   ($field["required"] === true ? "required" : "")
     ...
     foreach($list_source->get_list(array(),$list_source->get_name_field(),"",1000) as $list_field) {
